"""
Custom controller extending browser-use v0.12.0 Tools with MCP integration
...
class CustomController(Tools):
    def __init__(
        ...
        ask_assistant_callback: Optional[
            Union[
                Callable[[str, BrowserSession], Dict[str, Any]],
                Callable[[str, BrowserSession], Awaitable[Dict[str, Any]]],
            ]
        ] = None,
    ):
        super().__init__(
            exclude_actions=exclude_actions if exclude_actions is not None else [],
    ...
    async def act(
        ...
        """Execute an action, routing MCP tools to the MCP client."""
